...
method: 提出一种结合稀疏标注的教师-学生框架，通过EMA稳定教师网络、置信度过滤和课程引导细化来逐步优化伪掩码。
result: 在GlaS数据集上达到80.10的mIoU和89.10的Dice系数，并在TCGA等跨队列评估中表现出较强的鲁棒性。
conclusion: 该框架为结直肠组织病理学腺体分割提供了一种标注高效且泛化性强的解决方案。
---

## 摘要
背景与目标：结直肠癌的组织病理学分级依赖于腺体结构的准确分割。目前的深度学习方法依赖于大规模像素级标注，这在常规临床实践中既费力又难以获得。弱监督语义分割提供了一个极具前景的替代方案。然而，基于类激活图（CAM）的方法通常会产生不完整的伪掩码，这些掩码强调高度判别性的区域，而无法监督未标注的腺体结构。我们提出了一个弱监督教师-学生框架，该框架利用稀疏的病理学家标注和指数移动平均（EMA）稳定的教师网络来生成细化的伪掩码。方法：该框架集成了基于置信度的过滤、教师预测与有限真值的自适应融合，以及课程引导的细化，以逐步分割未标注的腺体区域。该方法在来自俄亥俄州立大学韦克斯纳医学中心的机构结直肠癌队列（包含60张苏木精-伊红染色的全切片图像）以及包括腺体分割数据集、TCGA COAD、TCGA READ和SPIDER在内的公共数据集上进行了评估。结果：在腺体分割数据集上，该框架实现了80.10的平均交并比（mIoU）和89.10的平均Dice系数。跨队列评估表明，在无需额外标注的情况下，该方法在TCGA COAD和TCGA READ上表现出鲁棒的泛化能力，而在SPIDER上性能的下降则反映了领域偏移。结论：所提出的框架为结直肠组织病理学中的腺体分割提供了一种标注高效且具有泛化能力的方法。
