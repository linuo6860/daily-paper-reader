...
tags: ["query:bio-ca"]
score: 6.0
evidence: 蛋白质-核苷酸结合位点预测的生物信息学方法
tldr: 本研究针对蛋白质-核苷酸结合位点预测中特征表示不足和融合机制僵化的问题，提出了MTGA-MGE框架。该框架通过多粒度编码网络提取判别性信号，利用遗传算法自适应进化任务特定的融合策略，并引入外部邻域机制促进跨任务协作学习。实验证明，该方法在15个数据集上均取得领先性能，尤其在低资源场景下表现稳健，为药物研发提供了有力支持。
selection_source: fresh_fetch
motivation: 现有计算方法在特征表示和任务融合机制上存在局限，难以有效挖掘跨任务信息的协同潜力。
method: 提出MTGA-MGE框架，集成多粒度编码、遗传算法驱动的自适应融合以及基于生物相似性的外部邻域协作机制。
result: 在15个核苷酸数据集上的广泛评估显示，该方法在多资源场景下均刷新了现有最优性能记录。
conclusion: MTGA-MGE为后基因组时代解码复杂的蛋白质-配体相互作用提供了一种高效且自适应的预测方案。
---
